...
第二台之後的裝置，打開 App 第一個畫面就有一格輸入框：

1. 貼上你的**同步密語**（在舊裝置 ⚙︎ 裡看得到）
2. 按「用密語接手」

完成。記憶、API 金鑰、臉部檔案、備用密碼全部下來，**直接進主畫面**，
之後就是開 App → 掃臉 → 用。

不用填 repo（App 從自己的網址推導）、不用填 GitHub token
（token 藏在加密內容裡，解開後自動就位）。真的只有那一組密語。

> **為什麼不能「掃臉就同步」？**
> 因為**臉不是秘密**。密碼是「你知道的東西」，臉是「你就是的東西」——
> 加密需要一個只有你知道的秘密，而你的臉任何人都看得到、拍得到。
> 如果掃臉就能解開雲端資料，那別人拿你一張照片也能解開。
> 所以臉負責「開你手上這台已經認得你的手機」，密語負責「讓新手機第一次認識你」。
> 一台裝置只需要做一次。
...
**公開的東西會被存檔**，萬一哪天密語外流，別人手上那份舊檔就能回頭解開。
...
因為 repo 是公開的、檔案任何人都下載得到，**密語就是唯一擋在資料前面的東西**：

- 用 App 產生的那組（160 bits 隨機）最安全，抄在安全的地方就好
- 想改成自己記得住的句子也可以，但**請至少 12 個字**，不要用生日或常見密碼——
  公開的檔案可以被拿去離線慢慢猜
- 換成 private repo 的話，攻擊者還得先有 token，密語的壓力就小很多

另外要知道：為了讓新裝置只憑密語就能接手，**GitHub token 也放在加密內容裡**。
好處是換手機不用重弄 token，代價是每台裝置解開後都持有寫入權限——
所以裝置掉了就去 GitHub 把那把 token 撤銷、重新產生一把。
