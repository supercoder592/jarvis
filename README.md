...
- **跨裝置搬家** — 兩種方式：匯出加密備份檔用 AirDrop 傳（不用帳號、不用 token，
  連 API 金鑰都一起過去），或接上 GitHub 做背景自動同步（新裝置掃臉就接手）
...
- **讓 JARVIS 自己記下重要的事** — 預設開啟，記到新東西會在一兩秒內同步到其他裝置。它會在回覆裡夾帶記憶指令，
...
### 換新裝置（不用 GitHub）：AirDrop 一個檔案

完全不需要 token、不需要帳號、不需要任何服務：

1. 舊裝置：⚙︎ →「**匯出備份檔**」→ 分享面板選 **AirDrop** 傳給新裝置
2. 新裝置：打開 App →「**從備份檔接手**」→ 選那個檔案

完成。而且因為這個檔案是你自己傳給自己的、不經過任何公開場所，
**連 API 金鑰都會一起過去**，新裝置什麼都不用再設定。

缺點是手動：它是「搬家」不是「持續同步」。想要背景自動同步才需要下面那套。

### 換新裝置（自動同步）：掃臉就好

第二台之後的裝置，打開 App 第一個畫面就有一顆「**掃臉接手**」：

1. 按下去
2. 看鏡頭

完成。記憶、個性、臉部檔案、備用密碼全部下來，直接進主畫面。
不用打字、不用密語、不用填 repo。

運作方式：App 從自己的網址知道資料放在哪，先把同步檔抓下來（**還不寫進本機**），
拿裡面的臉部檔案當比對基準驗你的臉，通過了才真的套用。

**API 金鑰預設也會跟著跑**（⚙︎ 可以關掉）。這是個取捨：
新裝置接手完就能直接用，但金鑰跟著進了公開的同步檔——最壞情況是免費額度
被別人用掉，重新產生一把即可。**只有在金鑰沒綁付款方式時才適合這樣。**
綁了信用卡的金鑰請關掉那個開關。

**GitHub token 也會跟著同步**，否則新裝置只讀得到、寫不回去——那樣 B 裝置
學到的事永遠回不到 A 裝置，不算真正的同步。

所以有一件事要做對：**同步資料放在一個獨立的 repo**，token 只給那一個 repo
的 Contents 權限。這樣就算 token 外流，別人也只能弄壞那份資料（刪掉重建即可），
碰不到這個 App 本身。**不要把 token 開在 App 所在的 repo 上**——那條 `main`
分支就是網站本體。

> **為什麼不能連金鑰也一起掃臉帶過去？**
> 因為**臉不是秘密**。密碼是「你知道的東西」，臉是「你就是的東西」——
> 要讓新手機自己解開檔案，解密方式就得寫在 App 原始碼裡，而原始碼是公開的。
> 所以那個檔案能放的東西，就是「被看到也還好」的東西。
> 臉在這裡的角色是一道門（擋住隨手拿你手機的人），不是一把鎖。

想要真正的加密，到 ⚙︎ →「進階」填一組同步密語，那時整份檔案就只有你解得開，
金鑰也可以放心跟著跑——代價是每台新裝置要輸入一次。
...
**公開的東西會被存檔**——同步檔沒設密語的話，等於「被看到也還好」的那些東西是公開的。
...
| 長期記憶、稱呼、個性指令 | 對話紀錄 |
| 供應商與模型選擇 | 語音、語速、朗讀開關 |
| 臉部辨識檔案（可關閉） | Repo／分支／路徑 |
| 備用密碼（存的是加鹽雜湊） | 辨識嚴格度、眨眼偵測 |
| API 金鑰與 GitHub token（可關閉） | |
...
- **沒設密語時**：檔案是 AES-256-GCM 密文，但解密方式寫在公開的原始碼裡，
  所以對「願意翻原始碼的人」等於明文。它擋得住的是掃 GitHub 撿 API 金鑰的自動爬蟲，
  而**同步檔裡本來就沒有金鑰**——這是刻意設計的，讓「被看到」的代價降到只有隱私
- **有設密語時**：金鑰用 PBKDF2-SHA256（25 萬次）從密語導出，只有你解得開。
  密語沒了就救不回來，我沒有後門
- **Token 被偷的話**：對方可以刪掉或覆蓋那個檔案。
  請用 fine-grained token 且只給一個 repo 的 Contents 權限，並設到期日
- Token 存在瀏覽器的 localStorage，跟 API 金鑰一樣——能存取你手機瀏覽器資料的人就拿得到
