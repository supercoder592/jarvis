...
密文再怎麼加雜訊也不會更難破解（AES-GCM 的輸出本來就跟亂數無異），
但**元資料**會洩漏，這部分有處理：

- **內容多寡看不出來**：加密前先用隨機資料把明文補滿到 8KB 的整數倍，
  所以記了 0 件事和記了 30 件事，上傳的檔案**大小完全一樣**，
  旁人無法從檔案變大推測你今天多記了什麼
  （補的長度是用 UTF-8 位元組算的——用字元數會漏掉中文一字三位元組這件事）
- **不寫演算法名稱**：檔案長這樣 `{"v":2,"s":"…","i":"…","d":"…"}`，
  沒有 `alg: PBKDF2-SHA256/AES-256-GCM` 這種自報家門的欄位
- **commit 訊息固定是 `update`**，不會在公開的 commit 歷史裡寫「JARVIS 同步」
- **預設檔名 `data.json`**，不叫 jarvis-something

還是看得出來的：檔案在 8KB 還是 16KB 這個級距（等於「有沒有存臉部檔案」），
以及 commit 時間點（等於你何時用了 App）。要連這個都藏起來得付出不成比例的代價。
